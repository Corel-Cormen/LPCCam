...
/*	communication peripherals rw functions	*/
...
static status_t send_i2c(uint8_t addr, uint8_t * data, uint8_t data_len)
{
	...
	return I2C_MasterTransferBlocking(FLEXCOMM4_PERIPHERAL, &i2c_tr);
}
...
static status_t send_and_read_spi(uint8_t * tx_data, uint8_t tx_len, uint8_t * rx_data)
{
	...
	return SPI_MasterTransferBlocking(FLEXCOMM8_PERIPHERAL, &spi_tr);
}
...
//		  B_CS  = digitalPinToBitMask(CS);
  ...
  // pinMode(CS, OUTPUT);
 #else
//	  gpio_pin_config_t config = { kGPIO_DigitalOutput, 1 };	/* HARDWARE SSEL */
//	  GPIO_PinInit(GPIO, 1, 1, &config);						/* HARDWARE SSEL */
	#endif
	sensor_model = model;
	switch (sensor_model)
	{
...
void ArduCAM::set_fifo_burst()
{
	...
	#else
	uint8_t send_data[] = { BURST_FIFO_READ };
	send_spi(send_data, ARRAY_SIZE(send_data));
...
byte ArduCAM::wrSensorReg8_8(int regID, int regDat)
{
	...
	#else
		uint8_t send_data[] =
			{ (regID & 0x00FF), (regDat & 0x00FF) };
		send_i2c(sensor_addr, send_data, ARRAY_SIZE(send_data));
...
{	
	...
	send_i2c(sensor_addr, send_data, ARRAY_SIZE(send_data));
	read_i2c(sensor_addr, read_data, ARRAY_SIZE(read_data));
...
byte ArduCAM::wrSensorReg8_16(int regID, int regDat)
{
	...
	#else
		uint8_t send_data[] =
			{ (regID & 0x00FF), (regDat >> 8), (regDat & 0x00FF) };
...
byte ArduCAM::rdSensorReg8_16(uint8_t regID, uint16_t* regDat)
{
  	...
  	*regDat = (read_data[0] << 8) | read_data[1];
...
byte ArduCAM::wrSensorReg16_8(int regID, int regDat)
{
	  ...
	  uint8_t send_data[] =
	  	  { (regID >> 8), (regID & 0x00FF), (regDat & 0x00FF) };
...
byte ArduCAM::wrSensorReg16_16(int regID, int regDat)
{
	...
	uint8_t send_data[] =
	{ (regID >> 8),  (regID & 0x00FF), (regDat >> 8), (regDat & 0x00FF) };

	if(send_i2c(sensor_addr, send_data, ARRAY_SIZE(send_data)) != kStatus_Success)
	{
		return 0;
	}
